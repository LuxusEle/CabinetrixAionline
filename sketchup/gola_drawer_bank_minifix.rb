...
module CabinetrixGolaDrawerBank
  ...
  def self.get_materials(model)
    ...
    # Vivid Hardware Materials for instant recognition
    ...
    accent_mat.color = Sketchup::Color.new(235, 75, 30) # Bright Orange/Red Indicator Arrow
  ...
  def self.build_minifix_joint(parent_ents, bolt_center, dir_vector, mats, face_z_offset = 9.0.mm)
    # bolt_center is at the interface point (on the side panel face)
    # dir_vector points along the panel thickness into the butted panel (+X or -X)
    ...
    # Cam top is on the interior surface of the horizontal board
    cam_top_z = cam_z + face_z_offset
    cam_top_pt = Geom::Point3d.new(cam_x, cam_y, cam_top_z + 0.2.mm) # +0.2mm ensures 100% visibility over wood face

    # 2a. Outer Dark Bore Cavity Ring (Ø15.4mm x 0.1mm)
    ...
    # 2d. Orange / Red Directional Arrow Indicator (points towards edge)
    ...
    # 3. Fluted Beech Alignment Dowel (Ø8x30mm, placed 32mm away in System 32 pitch)
  ...
  def self.build_gola_profile(parent_ents, profile_type, length, origin, mats)
    ...
    # The Gola channel / finger recess must OPEN toward the FRONT (-Y). The
    # profile is a Y-Z cross-section extruded along X. Mirror the contour about
    # its own centre so the span stays [oy-27.2, oy] but the opening flips to -Y.
  ...
  def self.build_2x_drawer_bank(parent_ents, origin, mats, mode: :hybrid)
    # Modes:
    #   :hybrid     -> Left Bank Drawers OPEN (revealing inside joinery), Right Bank CLOSED
    #   :joinery    -> Carcase Only (Drawers hidden) for 100% complete hardware inspection
    #   :closed     -> Both banks closed
    #   :exploded   -> Exploded joinery view (panels offset by 40mm)
    ...
    # Add 4x Minifix Sets per Bottom Panel (2 at front, 2 at rear)
    [b1_ox, b2_ox].each do |bay_x|
      # Left edge of bay bottom panel
      ...
      # Right edge of bay bottom panel
    ...
    rail_h = 80.0.mm
    [b1_ox, b2_ox].each do |bay_x|
      # 1. Rear Horizontal Top Stretcher (18x80mm)
      ...
      # Minifix on Top Rear Stretcher (Facing downwards/upwards for easy assembly)
      ...
      # 2. Front Sub-rail behind L-Gola (18x80mm flat under top)
    ...
    unless mode == :joinery
      ...
      [0, 1].each do |bank_idx|
        ...
        # Determine pull extension based on mode:
        # If hybrid: Bank 0 (Left) is OPEN 300mm, Bank 1 (Right) is CLOSED 0mm!
...
# ------------------------------------------------------------------------------
# SKETCHUP MENU INTEGRATION
# ------------------------------------------------------------------------------
if defined?(UI) && UI.respond_to?(:menu)
  # Only register the menu when loaded as a standalone plugin (not when a build
  # script loads this file via CABINETRIX_NO_AUTORUN for geometry-only).
  unless (defined?(CABINETRIX_NO_AUTORUN) && CABINETRIX_NO_AUTORUN) || file_loaded?(__FILE__)
...
# Auto-run when loaded directly in SketchUp
